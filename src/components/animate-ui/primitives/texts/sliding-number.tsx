...
function SlidingNumberRoller({
  ...
  const [measureRef, { height }] = useMeasure({
    polyfill: typeof window !== 'undefined' && typeof window.ResizeObserver !== 'undefined' ? undefined : SafeResizeObserver as any,
  });
...
function SlidingNumberDisplay({
  ...
  if (!height) {
    const isTarget = Math.floor(motionValue.get()) % 10 === number;
    return (
      <span
...
function SlidingNumber({
  ...
  decimalPlaces = 0,
  thousandSeparator,
  transition = { stiffness: 280, damping: 28, mass: 0.3 },
  delay = 0,
